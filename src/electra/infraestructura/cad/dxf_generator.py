"""DXF architectural plan generator from JSON layout."""
...
def generate_dxf(data: dict, output_path: str) -> str:
    import ezdxf
    ...
    gray = 0x666666
    light_gray = 0x999999
    doc.layers.new("MUROS", dxfattribs={'color': 7, 'true_color': black, 'lineweight': 35})
    ...
    doc.layers.new("MARCO", dxfattribs={'color': 7, 'true_color': black, 'lineweight': 50})
    ...
    for room in data["rooms"]:
        ...
        if draw_room_walls and room.get("draw_walls", True):
            walls.add(_segment_key((rx, ry), (rx + rw, ry)))
            walls.add(_segment_key((rx, ry + rh), (rx + rw, ry + rh)))
            walls.add(_segment_key((rx, ry), (rx, ry + rh)))
            walls.add(_segment_key((rx + rw, ry), (rx + rw, ry + rh)))
    ...
    if "ignored_walls" in data:
        for p in data["ignored_walls"]:
            p1, p2 = p
            ignored_walls.add(_segment_key(p1, p2))
...
def generate_pdf_from_dxf(dxf_path: str) -> str | None:
    try:
