#!/usr/bin/env python3
"""Cotizador automÃ¡tico multi-proveedor para materiales elÃ©ctricos."""
...
def construir_comparativa(
    ...
    for idx, material in enumerate(materiales, 1):
        ...
        for slug, proveedor in proveedores.items():
            ...
            # FASE 2: BÃºsqueda con equivalentes comerciales
            ...
            # FASE 3: BÃºsqueda con superior inmediato
            ...
            if fase3_info:
                ...
                # Si nada funcionÃ³, agregamos los de Fase 1 (fallbacks)
                resultados_item.extend(resultados)
...
def _url_tag(url: str) -> str:
    return '<a href="%s">url</a>' % html.escape(url)
        ...
        for r in comp.resultados_por_proveedor:
            ...
            <tr>
              ...
              <td>{r.score_total:.2f}</td>
               <td>{_url_tag(r.url) if r.url else '-'}</td>
            </tr>""")
...
def escribir_resumen_md(path: Path, raw_bom: Dict[str, Any], comparativas: List[ComparativaItem], resumen: Dict[str, Any]) -> None:
    lines = [
        "# Resumen de CotizaciÃ³n Multi-Proveedor",
        "",
        f"**Proyecto:** {raw_bom.get('proyecto', 'N/E')}",
        f"**Propietario:** {raw_bom.get('propietario', 'N/E')}",
        f"**Fecha de generaciÃ³n:** {datetime.now().isoformat(timespec='seconds')}",
        "",
        "## Resumen de Totales",
        "",
        f"- **Total de Compra Real Mixta (unidades comerciales):** S/ {resumen.get('total_compra_real_mixta', 0.0):.2f}",
        f"- **Total Recomendado Neto (valores de diseÃ±o):** S/ {resumen.get('total_recomendado_materiales', 0.0):.2f}",
    ...
    lines.extend([
        ...
        "",
        "Esta tabla presenta el costo total de compra real en cada tienda (considerando empaques de rollos, tubos y cantidades comerciales) versus la mejor opciÃ³n mixta recomendada.",
        "",
        "| Material | Cant. | Unidad | Promart | Sodimac/Maestro | MercadoLibre | Mejor OpciÃ³n | ObservaciÃ³n |",
        "|---|---|---|---|---|---|---|---|",
    ])
...
def generar_informe_latex_imprimible(
    ...
    _pendientes_fallback = r'\multicolumn{3}{c}{No quedaron materiales pendientes.} \\'
    _itemize_lines = "\n".join(
...
{chr(10).join(pendientes_rows) if pendientes_rows else _pendientes_fallback}
\bottomrule
\end{{longtable}}
...
def generar_md_tienda(
    ...
    lines = [
        f"# Reporte de CotizaciÃ³n - {title}",
        "",
        "| Material BOM | Producto Encontrado | Cant. Req. | Und. Dis. | Und. Com. | Cont. Com. | Cant. Com. a Comprar | Precio Com. | Precio Equiv. | Subtotal Real | Sobrante | URL | Conf. | ObservaciÃ³n |",
        "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|",
    ]
...
def generar_md_recomendada_mixta(path: Path, comparativas: List[ComparativaItem], resumen: Dict[str, Any]) -> None:
    lines = [
        "# Reporte de CotizaciÃ³n Recomendada Mixta",
        "",
        "## Resumen General",
        "",
    ...
    for fila in resumen.get("plan_compra_consolidado", []):
        if fila.get("proveedor"):
            ...
            lines.append(
                f"| {fila['descripcion_representativa']} | {fila['partidas_bom']} | *Pendiente de revisiÃ³n* | - | "
...
def generar_md_enlaces_evidencias(path: Path, comparativas: List[ComparativaItem]) -> None:
    lines = [
        "# Enlaces de VerificaciÃ³n de Productos",
        "",
        "| Material BOM | Proveedor | Producto | Precio | Unidad Comercial | Fecha consulta | Tipo Coincidencia | Estado | Confianza | ObservaciÃ³n | URL |",
        "|---|---|---|---|---|---|---|---|---|---|---|",
    ]
...
def generar_md_pendientes(path: Path, comparativas: List[ComparativaItem]) -> None:
    ...
    lines = [
        "# Reporte de Materiales Pendientes de RevisiÃ³n",
        "",
        "Este reporte lista los materiales del BOM que no pudieron cotizarse automÃ¡ticamente de forma confiable, agrupados por su causa, sugiriendo una acciÃ³n correctiva.",
        "",
    ]
    
    causas = [
        ("no_encontrado", "1. No encontrado en proveedores", "No se encontraron precios en la web para el tÃ©rmino buscado."),
        ("no_convertible", "2. Encontrado pero unidad comercial no convertible", "Los productos encontrados usan unidades comerciales que no tienen regla de conversiÃ³n confiable en `unidades_comerciales.json`."),
        ("baja_confianza", "3. Encontrado pero confianza baja", "Los productos se encontraron pero su score tÃ©cnico o total es inferior a 0.45."),
        ("similar_revision", "4. Producto similar, requiere revisiÃ³n", "Se encontrÃ³ un producto similar o equivalente superior, pero por seguridad requiere aprobaciÃ³n manual."),
        ("bloqueo_datos", "5. Proveedor bloqueÃ³ o no expuso datos", "El scraping fallÃ³ o devolviÃ³ errores de red/bloqueo."),
        ("falta_regla", "6. Falta regla de equivalencia", "No existe regla para buscar equivalentes de esta categorÃ­a.")
    ]
    ...
    for key, title, desc in causas:
        ...
        lines.append("")
        if not items:
            lines.append("*NingÃºn material en esta secciÃ³n.*")
        else:
            lines.append("| Material | Cantidad Requerida | Detalle | AcciÃ³n Propuesta |")
            lines.append("|---|---|---|---|")
...
def generar_md_readme(path: Path, raw_bom: Dict[str, Any], resumen: Dict[str, Any]) -> None:
    lines = [
        f"# CotizaciÃ³n de Materiales - {raw_bom.get('proyecto', 'Proyecto')}",
        "",
        "Este directorio contiene los entregables de la cotizaciÃ³n multi-proveedor del proyecto.",
        "",
        "## Estructura de Salidas",
        "",
        "- `comparativa_precios.json`: Base de datos completa en JSON.",
        "- `comparativa_precios.csv`: Tabla de comparativas en formato CSV.",
        "- `comparativa_precios.xlsx`: Hoja de cÃ¡lculo Excel organizada con pestaÃ±as por tienda.",
        "- `cotizacion_recomendada.html`: CotizaciÃ³n recomendada en HTML interactivo.",
        "- `cotizacion_recomendada.tex`: Tabla LaTeX lista para insertar en el capÃ­tulo II del expediente.",
        "- `resumen_cotizacion.md`: Resumen general de la cotizaciÃ³n y totales.",
        "- `pendientes_revision.md`: Reporte detallado de materiales que requieren validaciÃ³n manual.",
        "- `evidencias/`: Carpeta con evidencias de scraping y enlaces de verificaciÃ³n.",
        ...
        "",
        "## Resumen de la CotizaciÃ³n Mixta Recomendada",
        "",
        f"- **Total de compra real mixta (unidades comerciales):** S/ {resumen.get('total_compra_real_mixta', 0.0):.2f}",
